...
def coeff_NL(eps, d6):
    """
    Coefficients du modèle en couches Hall-MHD étendu au secteur non local m = 2.
    lam = 2**(1/eps) 
    d6 est un parametre libre, mais a 0 on retrouve le modele a deux couches (m = 1)
    """
    lam = 2**(1/eps)
    ...
    return beta, zeta, gamma, p, q, r, delta4, delta5, delta6


def NL(V, B, parametre):
    """
    Par rapport a avant, on rajoute un 0 de chaque coté pour ne pas taper le bord avec le fait que l'on prend une couche plus loin
    """
    N, k, di, eps, d6 = parametre[0], parametre[2], parametre[6], parametre[12], parametre[15]

    V_grand = np.zeros(N+6, dtype = complex)  # portée max = 3 couches donc 3 zéros de chaque côté
    B_grand = np.zeros(N+6, dtype = complex)
    V_grand[3: N+3] = V  # On a donc V_grand = [0, 0, 0, V, 0, 0, 0]
    B_grand[3: N+3] = B

    Vn1 = V_grand[4: N+4] #Vn+1
    Vn2 = V_grand[5: N+5] #Vn+2
    Vn3 = V_grand[6: N+6] #Vn+3
    Vm1 = V_grand[2: N+2] #Vn-1
    Vm2 = V_grand[1: N+1] #Vn-2
    Vm3 = V_grand[0: N]   #Vn-3
    ...
    nonL1 = Vn1*Vn2 - Bn1*Bn2     #différentes contributions non linéaires locales (triades n, n+1, n+2 etc)
    nonL2 = (Vm1*Vn1 - Bm1*Bn1)
    nonL3 = (Vm1*Vm2 - Bm1*Bm2)
    nonL4 = (Vn2*Vn3 - Bn2*Bn3)   # triades non locales m = 2 : (n, n+2, n+3)
    nonL5 = (Vm2*Vn1 - Bm2*Bn1)   #                             (n-2, n, n+1)
    nonL6 = (Vm3*Vm1 - Bm3*Bm1)   #                             (n-3, n-1, n)

    croiseNL1 = (Vn1*Bn2 - Bn1*Vn2)
    croiseNL2 = (Vm1*Bn1 - Bm1*Vn1)
    croiseNL3 = (Vm2*Bm1 - Bm2*Vm1)
    croiseNL4 = (Vn2*Bn3 - Bn2*Vn3) #pareil non locales
    croiseNL5 = (Vm2*Bn1 - Bm2*Vn1)
    croiseNL6 = (Vm3*Bm1 - Bm3*Vm1)

    beta, zeta, gamma, p, q, r, delta4, delta5, delta6 = coeff_NL(eps, d6)

    NLV = 1j*k*np.conj(nonL1 + beta*nonL2 + zeta*nonL3 + p*nonL4 + q*nonL5 + r*nonL6) # équatiob (4) étendue, pour les N couches (a temps fixé)

    terme1 = 1j*k*np.conj( gamma*(croiseNL1 + croiseNL2 + croiseNL3) + delta4*croiseNL4 + delta5*croiseNL5 + delta6*croiseNL6) # équation (5) étendue
    terme2 = signe(N)*di*1j*k**2*np.conj(Bn1*Bn2 + Bm1*Bn1*beta + Bm2*Bm1*zeta + Bn2*Bn3*p + Bm2*Bn1*q + Bm3*Bm1*r) # terme de Hall
...
def integ(v0, b0, parametre, nb_sauvegarde, V_nul):
    T_max, k, nu, eta, constante, activer = parametre[1], parametre[2], parametre[7], parametre[8], parametre[14], parametre[16]
