...
def CI(parametre):
    ...
    for i in range(N):
        enveloppe = np.exp(-((i/eps-centre)**2) / (2 * sigma_k**2))  # gaussienne en n
...
def visu_casc(simulations, labels, pentes, mure):
    ...
    for (V, B, parametre, l_T), label, pente, mur in zip(simulations, labels, pentes, mure):
        EkV, EkB = E_kn(V, B, parametre)
        Ek = EkB + EkV #np.log10(EkB + EkV)
        R = 1000
        D = len(Ek[1])
        Ek_r = np.zeros((R,D))
        l_T_r= np.zeros(R)
        for i in range(R):
            Ek_r[i] = Ek[round(i*len(l_T)/R)]
            l_T_r[i]= l_T[round(i*len(l_T)/R)]
        k = parametre[2]
        fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
        k_casc, T_casc = np.meshgrid(np.log10(k),l_T_r)
        print(Ek[1])
        print(Ek[-1])
        # surf = ax.plot_surface(k_casc, T_casc, Ek_r, cmap="flag",linewidth=0.1, antialiased=True)
        surf = ax.plot_wireframe(k_casc, T_casc, Ek_r, color='C1', axlim_clip=True)
        ax.set_ylabel("temps t")
        ax.set_xlabel("nombre d'onde (log)")
        ax.set_zlabel("Spectre d'energie (log)")
        #ax.set_zlim3d(bottom=-10, top=1) 
        ax.set_xlim3d(left=-3, right= 0) 
        # fig.colorbar(surf)
        
        # ls = LightSource(270, 45)
        # # To use a custom hillshading mode, override the built-in shading and pass
        # # in the rgb colors of the shaded surface calculated from "shade".
        # rgb = ls.shade(Ek_r, cmap=plt.colormaps["gist_earth"], vert_exag=0.1, blend_mode='soft')
        # ax.plot_surface(k_casc, T_casc, Ek_r, rstride=1, cstride=1, facecolors=rgb,
