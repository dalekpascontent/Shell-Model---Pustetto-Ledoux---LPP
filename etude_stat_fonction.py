...
def show_Inv(simulations):
    fig, axs = plt.subplots(3, sharex=True)
    for (V, B, parametre, l_T) in simulations:
        l_E, l_Hm, l_Hh = invariant(V, B, parametre)
        axs[0].plot(l_T, l_E, markersize=3)
        axs[1].plot(l_T, l_Hm, markersize=3)
        axs[2].plot(l_T, l_Hh, markersize=3)
    axs[0].set_ylabel("E")
    axs[1].set_ylabel("Hm")
    axs[2].set_ylabel("Hh")
    axs[2].set_xlabel("t")
    ...
    fig.suptitle("Comparaison des invariants pour un même régime (énergie, hélicité magnétique et hybride)")
    fig.tight_layout()

def show_spectre(simulations, pente, mure):
    plt.figure()
    liste_Ek = []
    for (V, B, parametre, l_T), mur in zip(simulations, mure):
        k = parametre[2]
        Ek = moy_E(V, B, parametre, l_T, mur)
        Ek = liss_E(Ek)
        liste_Ek.append(Ek)
        y = Ek*k**(-pente)
        plt.plot(k, y, markersize=4)
    ...
    plt.title("Comparaison des spectres d'énergie pour un même régime")
    plt.ylim(10**-25, 10**2)
    plt.legend(fontsize=8)
    return liste_Ek

def etude_stat(liste_Ek, parametre, pente):
    plt.figure()
    k = parametre[2]
    moyenne = np.mean(liste_Ek, axis = 0)
    y = moyenne*k**(-pente)
