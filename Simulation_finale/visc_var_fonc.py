...
def tps_cascade(V, B, parametre, l_T, delta):
    ...
    On crée un seuil d'énergie sous lequel on considère que la cascade a deja démarrée. On regarde ensuite l'indice correspondant a ce moment.
...
def show_Inv(inv_tot, labels):
    ...
    for regime, label in zip(inv_tot, labels):
        ...
        for inv in regime:
            ...
            if preimere_courbe:
                plot1 = axs[0].plot(l_T, l_E, label=label, markersize=3)
                color = plot1[0].get_color()
                axs[1].plot(l_T, l_Hm, color=color, label=label, markersize=3)
                axs[2].plot(l_T, l_Hh, color=color, label=label, markersize=3)
...
def show_EVB(simulations, labels, pentes, kdi, l_tau_tc):
    ...
    for (Eu, Eb, parametre), label, pente, k_di, tau_tc in zip(simulations, labels, pentes, kdi, l_tau_tc):
        ...
        if nom in ["HMHD_s", "HMHD_b"]:
            ...
            plt.plot(k, yB, linewidth=1, marker='^', fillstyle='none', markeredgecolor='black', markersize=6, label=label)
            plt.plot(k, [1e5 for i in range(len(k))], '--')
            plt.plot(k, yV,  linewidth=1, marker='x', fillstyle='none', markeredgecolor='black', markersize=6, label=label)
            ...
            plt.plot(k, y,  linewidth=1, marker='^', fillstyle='none', markeredgecolor='black', markersize=6, label=label)
    ...
    return package, liste_Eu, liste_Eb


def ratio_EB(package):
    Eu, Eb, k, label = package[0], package[1], package[2], package[3]
    plt.figure()
