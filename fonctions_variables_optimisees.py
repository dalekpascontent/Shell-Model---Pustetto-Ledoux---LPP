...
def integ(v0, b0, parametre, nb_sauvegarde, V_nul):
    ...
    while t < T_max:
        ...
        if compteur ==0: # Euler pour la première intégration
            ...
            B_2 = Bmain*expB + dt*((1+omega/2)*NLB*expB - (omega/2)*fapB*expB*exp_m1_B)
...
def run_simu(parametre, nb_sauvegarde, V_nul):
    v0, b0 = CI(parametre)
    V, B, l_T = integ(v0, b0, parametre, nb_sauvegarde, V_nul)
    return V, B, l_T
...
def E_kn_VB(V, B, parametre):
    k = parametre[3]
    return np.abs(V)**2/(2*k), np.abs(B)**2/(2*k)
...
def moy_Ek_VB(V, B, parametre, l_T):
    T_max = parametre[1]
    time_moy = parametre[4] 
    P = V.shape[0]

    EkV, EkB = E_kn_VB(V, B, parametre)
    Ekn = EkV + EkB
    ...
    moy_E_k_zone_B = np.mean(EkB[debut:fin], axis=0)    

    return moy_E_k_zone_V, moy_E_k_zone_B
