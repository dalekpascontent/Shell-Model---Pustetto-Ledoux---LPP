...
def tps_cascade(V, B, parametre, l_T, delta):
    ...
    amplitude_0 = np.sqrt(np.abs(V[0][0])**2 + np.abs(B[0][0])**2)
...
""" ########################################################## les fonctions ne fonctionnent actuellemnt plus a cause des changements
def show_Eu_Eb(liste_Eu, liste_Eb, labels):
    plt.figure()

    for Eu,  Eb, label in zip(liste_Eu, liste_Eb, labels):
        plt.plot(l_T, Eu, color = "red", label=f"{label}, $E^u$")
        plt.plot(l_T, Eb, "--", color = "red", label=f"{label}, $E^b$")
    
    plt.xlabel("temps")
    plt.ylabel("$E^u$,     $E^b$")
    plt.title("Comparaison des énergies cinétiques et magnétiques au cours du temps")
    plt.legend(fontsize=8)

def etude_stat(liste_EkV, liste_EkB, parametre, pente):
    plt.figure()
    k, cas = parametre[2], parametre[11]
    if cas == "HMHD_s":
        moyenneV = np.mean(liste_EkV, axis = 0)
        moyenneB = np.mean(liste_EkB, axis = 0)
        yV = moyenneV*k**(-pente["HD"])
        yB = moyenneB*k**(-pente["HMHD_s"])
        plt.plot(k, yB, label="HMHD, $E^b$", markersize=4)
        plt.plot(k, [np.mean(yB[15:20]) for i in range(len(k))], '--')
        plt.plot(k, yV, label = "HMHD, $E^u$", markersize=4)
        plt.plot(k, [np.mean(yV[7:15]) for i in range(len(k))], '--')
    else:
        moyenne = np.mean(liste_EkV, axis = 0)
        y = moyenne*k**(-pente)
